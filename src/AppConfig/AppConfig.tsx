...
import { Config, Integration, ParameterDefinition, ValidateParametersFn } from'@contentful/ecommerce-app-base';
...
export default class AppConfig extends React.Component<Props, State> {
  ...
  getInitialFieldSkuTypes = (fieldSkuTypes: FieldsSkuTypes) => {
    // Map old values to array and remove "Any" value
    const validSkuTypeIds = Object.values(SkuTypeIds) as string[];
    for (const [contentType, contentTypeFields] of Object.entries(fieldSkuTypes)) {
      for (const [fieldId, fieldValues] of Object.entries(contentTypeFields)) {
        let values = Array.isArray(fieldValues) ? fieldValues : [fieldValues];
        values = values.filter(value => validSkuTypeIds.includes(value));
        fieldSkuTypes[contentType][fieldId] = values;
      }
    }
    return fieldSkuTypes;
  }
    ...
    this.setState(
      {
        ...
        fieldSkuTypes: this.getInitialFieldSkuTypes((parameters as { skuTypes?: FieldsSkuTypes })?.skuTypes ?? {}),
        appReady: true,
      },
      () => app.setReady()
    );
  ...
  validateFieldSkuTypes = (fieldSkuTypes: FieldsSkuTypes) => {
    for (const [contentType, contentTypeFields] of Object.entries(fieldSkuTypes)) {
      for (const contentTypeFieldValues of Object.values(contentTypeFields)) {
        if (!Array.isArray(contentTypeFieldValues) || contentTypeFieldValues.length < 1) {
          return "Please select at least 1 SKU type in each field"
        }
      }
    }
  }
  ...
  onAppConfigure = () => {
    ...
    const fieldError = this.validateFieldSkuTypes(fieldSkuTypes);
    ...
    if (fieldError) {
      this.props.sdk.notifier.error(fieldError);
      return false;
    }
  ...
  onParameterChange = (key: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.currentTarget.type === 'checkbox' ? e.currentTarget.checked.toString() : e.currentTarget.value;
          ...
          <>
            ...
            <Form>
              {parameterDefinitions.map((def) => {
                ...
                return def.id.startsWith('enable') ? (
                  <FormControl key={key} id={key}>
                    <Checkbox
                      id={key}
                      isChecked={parameters[def.id] === "true"}
